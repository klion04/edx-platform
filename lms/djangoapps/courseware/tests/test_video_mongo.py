...
class VideoDescriptorTest(TestCase, VideoDescriptorTestBase):
    ...
    def setUp(self):
        ...
        video_transcript_data = dict(
            video_id='',
            language_code='ar',
            transcript='ext101.srt',
            provider='Cielo24',
            file_format='srt',
        )
        self.get_video_transcript_data = lambda video_id: dict(video_transcript_data, **{'video_id': video_id})
    ...
    def test_import_val_data_internal(self):
        ...
        video_transcript = get_video_transcript(video.edx_video_id, 'ar')
        for key, value in self.get_video_transcript_data('test_edx_video_id').items():
            self.assertEqual(value, getattr(video_transcript, key))
    ...
    def test_import_val_data_external(self):
        ...
        # verify transcript data
        video_transcript = get_video_transcript(external_video_id, 'ar')
        for key, value in self.get_video_transcript_data(external_video_id).items():
            self.assertEqual(value, getattr(video_transcript, key))
